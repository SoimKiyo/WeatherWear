...
window.addEventListener("DOMContentLoaded", (event) => {
    ...
    if (existingImg) {
        ...
        // Masquer l'icône et le texte d'instruction
    ...
    // Initialiser les sous-types et matières
    updateSubtypes();
    updateMaterials();
    updateColors();
});
...
function updateSubtypes() {
    ...
    subtypeSelect.innerHTML = '<option value="" disabled>Choisir un sous-type</option>';
    ...
    // Sélectionner la valeur existante (placeholder provenant de la DB) si présente
    const existingSubtype = subtypeSelect.getAttribute("data-existing-value");
    if (existingSubtype) {
...
function updateMaterials() {
    ...
    materialSelect.innerHTML = '<option value="" disabled>Choisir une matière</option>';
    ...
    // Sélectionner la valeur existante (placeholder provenant de la DB) si présente
    const existingMaterial = materialSelect.getAttribute("data-existing-value");
    if (existingMaterial) {
        materialSelect.value = existingMaterial;
    }
}
...
function updateColors() {
    ...
    colorSelect.innerHTML = '<option value="" disabled>Choisir une couleur</option>';
    ...
    // Sélectionner la valeur existante (placeholder provenant de la DB) si présente
    const existingColor = colorSelect.getAttribute("data-existing-value");
    if (existingColor) {
        colorSelect.value = existingColor;
    }
}
...
function previewImage(file) {
    const reader = new FileReader(); // Crée une nouvelle instance de FileReader pour lire le contenu du fichier
